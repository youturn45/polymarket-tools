...
class KellyMonitorParams(BaseModel):
    """Parameters for long-term Kelly position monitoring and rebalancing."""

    # Duration
    monitor_duration_hours: float = Field(
        default=24.0,
        description="Hours to monitor position before stopping",
        ge=0.1,
        le=168.0,  # Max 1 week
    )
    ...
    price_change_threshold_pct: float = Field(
        default=0.05,
        description="Trigger rebalance when price moves by this % (0.05 = 5%)",
        ge=0.01,
        le=0.5,
    )
    position_deviation_threshold_pct: float = Field(
        default=0.10,
        description="Trigger rebalance when position deviates by this % (0.10 = 10%)",
        ge=0.01,
        le=0.5,
    )
    periodic_check_interval_minutes: float = Field(
        default=15.0,
        description="Trigger periodic rebalance every N minutes",
        ge=1.0,
        le=120.0,
    )

    # Rebalancing limits
    min_rebalance_shares: int = Field(
        default=10,
        description="Don't rebalance if delta < this many shares",
        ge=1,
        le=1000,
    )
    max_rebalances_per_day: int = Field(
        default=20,
        description="Maximum rebalances per day to prevent runaway behavior",
        ge=1,
        le=100,
    )

    # Monitoring interval
    monitor_check_interval_seconds: float = Field(
        default=60.0,
    ...
    # Kelly parameters (reused during rebalancing)
    kelly_params: KellyParams = Field(
        description="Kelly parameters for position sizing during rebalancing",
    )

    # Stop condition
    cancel_orders_on_completion: bool = Field(
        default=True,
        description="Cancel all pending orders when monitoring ends",
    )
